...
class FinalDecision(Action):
    ...
    ## YOUR TURN
    Based on your analysis result, previous communication and requirments, make a final decision: Should the drone distribution needs to be adjusted? 
...
class SubRegion1Drone(Role):
    ...
    def __init__(self, scene: str, **data: Any):
        super().__init__(**data)
        self.scene = scene
        self.set_actions([MonitorTargets(scene), FinalDecision()])
        self._set_react_mode(react_mode=RoleReactMode.BY_ORDER.value)
        self._watch([UserRequirement, MonitorTargets, FinalDecision])
...
class CommandDrone(Role):
    ...
    def __init__(self, **data: Any):
        super().__init__(**data)
        self.set_actions([CommandDrones(), FinalDecision()])
        self._set_react_mode(react_mode=RoleReactMode.BY_ORDER.value)
        self._watch([UserRequirement, CommandDrones, FinalDecision])
        self.assessment_result = ""
        self.topic = "humanoid robots monitor mission"
...
# rosrun centralized_robots_centralized_drones two_round.py --idea 'humanoid robots monitor mission'
