...
class AssessPlatformFromDrone(Action):
    ...
    - The robotic arm platform is located on a calm ocean surface.
    - The drone's battery level is low and need to land for charging.
...
class AssessDroneFromPlatform(Action):
    ...
    Additional Information:
    - The operation is taking place during the daytime.
...
class SpeakAloud(Action):
    ...
    {context}
    ## YOUR TURN
    Based on the image analysis result and previous communication, provide feedback and suggest whether docking is advisable.
    Provide a concise response in no more than 80 words.
    """
    name: str = "SpeakAloud"
...
class DroneRobot(Role):
    ...
    def __init__(self, **data: Any):
        super().__init__(**data)
        self.set_actions([AssessPlatformFromDrone(), SpeakAloud(), FinalDecision()])
        self._set_react_mode(react_mode=RoleReactMode.BY_ORDER.value)
        self._watch([UserRequirement, AssessPlatformFromDrone, SpeakAloud, FinalDecision])
...
class PlatformRobot(Role):
    ...
    def __init__(self, **data: Any):
        super().__init__(**data)
        self.set_actions([AssessDroneFromPlatform(), SpeakAloud(), FinalDecision()])
        self._set_react_mode(react_mode=RoleReactMode.BY_ORDER.value)
        self._watch([UserRequirement, AssessDroneFromPlatform, SpeakAloud, FinalDecision])
        self.assessment_result = ""
        self.topic = "drone docking mission"
...
async def cooperate(idea: str, investment: float = 3.0, n_round: int = 2):
    ...
    for _ in range(n_round):
        ...
        message_pool.append(await drone_robot._act(SpeakAloud(), message_pool))
        message_pool.append(await platform_robot._act(SpeakAloud(), message_pool))
        message_pool.append(await drone_robot._act(SpeakAloud(), message_pool))
        message_pool.append(await platform_robot._act(SpeakAloud(), message_pool))
...
if __name__ == "__main__":
    fire.Fire(main)
# rosrun day_calm_ocean four_round.py --idea "drone docking mission"
