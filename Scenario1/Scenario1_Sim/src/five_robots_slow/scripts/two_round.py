...
class AssessGroundRobots(Action):
    ...
    - There are five cleaning robots in front of you.
    - The cleaning robots are moving slow.
...
class AssessAerialView(Action):
    ...
    Additional Information:
    - You have a clear view of five cleaning robots from above.
    - The cleaning robots are moving slow.
    - Humanoid robots take up twice as much space as cleaning robots.
...
class FinalDecision(Action):
    ...
    {context}
    ## YOUR TURN
    Based on the image analysis result and previous communication, make a final decision: Is it safe for humanoid robot to proceed?
...
class GroundRobot(Role):
    ...
    def __init__(self, **data: Any):
        super().__init__(**data)
        self.set_actions([AssessGroundRobots(), FinalDecision()])
        self._set_react_mode(react_mode=RoleReactMode.BY_ORDER.value)
        self._watch([UserRequirement, AssessGroundRobots, FinalDecision])
...
class AerialRobot(Role):
    ...
    def __init__(self, **data: Any):
        super().__init__(**data)
        self.set_actions([AssessAerialView(), FinalDecision()])
        self._set_react_mode(react_mode=RoleReactMode.BY_ORDER.value)
        self._watch([UserRequirement, AssessAerialView, FinalDecision])
        self.assessment_result = ""
        self.topic = "navigate through cleaning robots"
...
# rosrun five_robots_slow two_round.py --idea "navigate through cleaning robots"
